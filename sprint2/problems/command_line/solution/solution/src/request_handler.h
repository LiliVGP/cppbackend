...
namespace http_handler {
    ...
    public:
        explicit RequestHandler(model::Game& game, fs::path static_root, bool tick_enabled = false)
            : game_{ game },
            static_root_{ fs::absolute(fs::weakly_canonical(static_root)) },
            api_handler_{ game, tick_enabled } {
        }
    ...
    private:
        ...
        void HandleStaticRequest(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
            ...
            if (decoded_target == "/" || decoded_target.empty()) {
