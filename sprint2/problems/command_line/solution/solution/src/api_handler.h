...
namespace http_handler {
    ...
    class ApiHandler {
    public:
        explicit ApiHandler(model::Game& game, bool tick_enabled)
            : game_(game), tick_enabled_(tick_enabled) {
        }
    ...
    private:
        ...
        void HandleGameTick(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
            if (tick_enabled_) {
                return send(MakeErrorResponse(http::status::bad_request, "badRequest", "Invalid endpoint", req.version(), req.keep_alive()));
            }
            ...
            try {
                ...
                double delta_time = time_delta_ms / 1000.0;
