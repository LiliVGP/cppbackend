...
namespace model {
    ...
    // Сериализация LostItem::Id
    template <typename Archive>
    void serialize(Archive& ar, LostItem::Id& id, [[maybe_unused]] const unsigned version) {
        uint32_t val = *id;
        ar& val;
        id = LostItem::Id{ val };
    }

    // Сериализация LostItem
    template <typename Archive>
    void serialize(Archive& ar, LostItem& item, [[maybe_unused]] const unsigned version) {
        ar& item.id;
        ar& item.type;
        ar& item.position;
    }
...
namespace serialization {
    ...
    struct GameState {
        ...
        std::vector<LostItem> lost_items;          // потерянные предметы
        ...
        void LoadToModel(GameModel& model) {
            ...
            for (const auto& item : lost_items) {
