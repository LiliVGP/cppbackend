...
public:
    ...
    void operator()(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
        ...
        // Handle /api/v1/maps
        if (target.starts_with(kApiMaps)) {
            HandleMaps(req, send);
            return;
        }
...
private:
    ...
    void HandleMaps(http::request<Body, http::basic_fields<Allocator>>& req, Send&& send) {
        ...
        std::string_view suffix = target.substr(std::string_view(kApiMaps).size());
    ...
    void HandleState(http::request<Body, http::basic_fields<Allocator>>& req, Send&& send) {
        ...
        for (const auto& [id, info] : players) {
            ...
            player_obj["bag"] = json::value(std::move(bag_arr));
    ...
    void HandleAction(http::request<Body, http::basic_fields<Allocator>>& req, Send&& send) {
        ...
        auto* mutable_player = game_.GetMutablePlayer(player_id);
    ...
    void HandleTick(http::request<Body, http::basic_fields<Allocator>>& req, Send&& send) {
        ...
        try {
            ...
            send(MakeJsonResponse(http::status::bad_request, json::serialize(err_obj), req, req.keep_alive()));
            return;
        }
        ...
        // Notify tick callback for auto-save
        if (tick_callback_) {
            tick_callback_(static_cast<int>(time_delta));
        }
