...
std::vector<detail::BookInfo> View::GetBooks() const {
    ...
    pqxx::work w{connection_};
    auto query = "SELECT title, publication_year FROM books ORDER BY title ASC";
    for (auto [title, year] : w.query<std::string, int>(query)) {
...
std::vector<detail::BookInfo> View::GetAuthorBooks(const std::string& author_id) const {
    ...
    // Исправлено: сортировка по году ASC, затем по названию ASC
    auto query = "SELECT title, publication_year FROM books WHERE author_id = $1 ORDER BY publication_year ASC, title ASC";
