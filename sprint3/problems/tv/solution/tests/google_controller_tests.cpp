...
TEST_F(ControllerWithTurnedOffTV, OnSelectChannelWithoutChannelPrintsError) {
...
TEST_F(ControllerWithTurnedOffTV, OnSelectChannelWithExtraArgsPrintsError) {
    RunMenuCommand("SelectChannel 5 extra"s);
    ExpectOutput("Error: the SelectChannel command requires exactly one argument\n"sv);
    EXPECT_FALSE(tv_.IsTurnedOn());
}

TEST_F(ControllerWithTurnedOffTV, OnSelectChannelDoesNothingWhenTVIsOff) {
...
TEST_F(ControllerWithTurnedOffTV, OnSelectPreviousChannelDoesNothingWhenTVIsOff) {
    RunMenuCommand("SelectPreviousChannel"s);
    ExpectOutput("TV is turned off\n"sv);
    EXPECT_FALSE(tv_.IsTurnedOn());
}

TEST_F(ControllerWithTurnedOffTV, OnSelectPreviousChannelWithArgsPrintsError) {
...
TEST_F(ControllerWithTurnedOnTV, OnInfoPrintsChannel1Initially) {
    RunMenuCommand("Info"s);
    ExpectOutput("TV is turned on\nChannel number is 1\n"sv);
}

TEST_F(ControllerWithTurnedOnTV, OnSelectChannelSelectsValidChannel) {
    RunMenuCommand("SelectChannel 5"s);
    ExpectEmptyOutput();
    EXPECT_THAT(tv_.GetChannel(), testing::Optional(5));
}

TEST_F(ControllerWithTurnedOnTV, OnSelectChannelPrintsErrorForOutOfRangeChannel) {
    RunMenuCommand("SelectChannel 100"s);
    ExpectOutput("Channel is out of range\n"sv);
    EXPECT_THAT(tv_.GetChannel(), testing::Optional(1));
}

TEST_F(ControllerWithTurnedOnTV, OnSelectChannelPrintsErrorForInvalidChannel) {
    RunMenuCommand("SelectChannel abc"s);
    ExpectOutput("Invalid channel\n"sv);
    EXPECT_THAT(tv_.GetChannel(), testing::Optional(1));
}

TEST_F(ControllerWithTurnedOnTV, OnSelectChannelPrintsErrorForExtraArgs) {
    RunMenuCommand("SelectChannel 5 extra"s);
    ExpectOutput("Error: the SelectChannel command requires exactly one argument\n"sv);
    EXPECT_THAT(tv_.GetChannel(), testing::Optional(1));
}

TEST_F(ControllerWithTurnedOnTV, OnSelectPreviousChannelSwitchesToPrevious) {
    tv_.SelectChannel(5);
...
TEST_F(ControllerWithTurnedOnTV, OnSelectPreviousChannelWithoutPreviousDoesNothing) {
    RunMenuCommand("SelectPreviousChannel"s);
    ExpectEmptyOutput();
    EXPECT_THAT(tv_.GetChannel(), testing::Optional(1));
}
...
TEST_F(ControllerWithTurnedOnTV, SelectChannelThenSelectPreviousChannelWorks) {
    RunMenuCommand("SelectChannel 5"s);
